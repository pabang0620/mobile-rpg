...
## Codex 역할 운영
역할별 작업 지침은 `.codex/agents/README.md`와 그 폴더의 역할 파일을 따른다. 총괄은 현재 모듈의 범위와 완료 조건을 먼저 고정하고, 기본 순서인 `설계 검토 → 구현 또는 에셋 제작 → 독립 검증 → 인수인계`를 지킨다. 각 역할은 배정받은 한 모듈만 다루며 다른 역할의 권한을 임의로 넘지 않는다.
