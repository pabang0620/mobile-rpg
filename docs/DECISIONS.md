...
## 2026-09-23: 슬라임 왕국 5단계 — 기존 배치 초기화

사용자 요청에 따라 SlimeKingdom 생성기를 Modular64 전용 40×30 지역으로 교체한다.
남쪽 입구, 중앙 사냥터, 두 교량, 북서 보물 샛길, 북동 계단 유적, 북쪽 보스 공터를 새로 배치한다.
기존 씬 이름과 마을 전환 계약은 유지한다. 개별 타일과 별도 충돌 데이터로 재구성하며,
기존 맵 그림을 합성하거나 타일 겹침으로 선을 가리지 않는다.
몬스터 직렬화 초기화와 공유 GridMap 점유 갱신도 연결해 저장한 씬에서 이동/전투가 작동하도록 한다.
상세 배치·검증 경로는 `SLIME_KINGDOM_MODULAR.md`를 참조한다.

## 2026-09-22: Modular64를 향후 모든 맵의 불변 제작 규칙으로 승격

사용자 확정에 따라 `docs/MODULAR_TILESET_64.md`의 64×64 모듈 제작 계약을 특정 맵의 구현안이 아니라 프로젝트 전체 지형·맵 제작 규칙으로 승격했다. 완성 맵 이미지 직접 사용, 타일 확대·겹침을 통한 선 가리기, 장식 baked-in은 허용하지 않는다. 모든 신규/개편 맵은 연결 타일, 분리 레이어, 안정 ID, 독립 테스트 씬, RGBA 경계 자동 검수와 조립 미리보기를 거친다. 세부 강제사항은 최상위 `AGENTS.md`의 「지형·맵 제작 불변 규칙」을 따른다.
